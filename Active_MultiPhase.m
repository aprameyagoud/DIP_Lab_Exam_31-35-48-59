...
function Active_MultiPhase(Img)
%[FileName,PathName] = uigetfile('*.*','Select Second Image from Dataset');
sr=128;
%Img=imread(FileName);  
Img=(imresize(Img,[sr sr]));
...
% figure(4);
% imagesc(Img,[0 255]);colormap(gray);hold on; axis off;axis equal;
% [c,h] = contour(u(:,:,1),[0 0],'r','LineWidth',2);
% figure;
% 
% imshow(u(:,:,2));
% 
